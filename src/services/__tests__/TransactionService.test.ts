...
await db.categories.clear();
  ...
  it('bulkImport saves valid rows and skips invalid rows without failing the batch', async () => {
    const inputList = [
      {
        amount: 500,
        type: 'expense',
    ...
    const { inserted, failed } = await TransactionService.bulkImport(inputList as any[]);
    
    expect(inserted.length).toBe(2);
    expect(failed.length).toBe(2);
    
    expect(failed[0].reason).toBeTruthy();
    expect(failed[1].reason).toBeTruthy();

    const allTxs = await db.transactions.toArray();
    expect(allTxs.length).toBe(2);
    expect(allTxs[0].description).toBe('Valid Item 1');
    expect(allTxs[1].description).toBe('Valid Item 2');
  });
});
