...
import { X, Mic, Settings, Calendar, Plus, Sparkles, AlertCircle } from 'lucide-react';
...
export default function QuickEntryModal({ 
  ...
  const [type, setType] = useState<'expense' | 'income' | 'udhaar_give' | 'udhaar_take'>('expense');
    ...
    async function loadLastCategory() {
      if (type === 'expense' || type === 'income') {
        ...
        const lastEntry = await db.udhaarEntries
          .where('type').equals(type === 'udhaar_give' ? 'give' : 'receive')
          .reverse()
          .first();
        if (lastEntry) {
  ...
  const currentTypeCategories = useLiveQuery(
    () => db.categories.where({ type: type === 'expense' ? 'expense' : 'income', context: activeContext }).toArray(), 
    [type, activeContext]
  ) || [];
  ...
  const handleSave = async (closeAfterSave: boolean) => {
    ...
    if (type === 'expense' || type === 'income') {
      ...
      if (cust) {
        const balanceChange = type === 'udhaar_give' ? finalAmountInPKR : -finalAmountInPKR;
        await db.customers.update(cust.id!, { balance: cust.balance + balanceChange });
      }
    }
  ...
  const processSmartVoiceText = async (text: string) => {
    ...
    try {
       ...
       if (!apiKey) throw new Error("GEMINI_API_KEY is missing. Please add it in App Settings.");
       ...
       setSmartVoiceError(err.message || "Could not process voice input properly. Please enter manually.");
    } finally {
       setIsParsingVoice(false);
    }
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <button
               ...
               className={`ml-2 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-colors flex items-center gap-1 ${
                 isSmartVoiceMode ? 'bg-indigo-500 text-white' : 'bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30'
               }`}
            >
        ...
        {isSmartVoiceMode ? (
          <div className="px-6 pb-8 pt-4 flex flex-col items-center justify-center min-h-[300px]">
             {isParsingVoice ? (
               ...
               <div className="flex flex-col items-center gap-6 w-full text-center">
                 <button
                    onClick={isRecording ? () => recognitionRef.current?.stop() : startSmartVoiceRecording}
                 ...
                 <div>
                   <h3 className="text-white font-bold text-lg mb-1">{isRecording ? "Listening..." : "Tap Mic to Speak"}</h3>
                   <p className="text-slate-400 text-sm max-w-[250px] mx-auto leading-relaxed">
                     Say something like<br/> <span className="italic text-slate-300">"500 rupees for petrol"</span> or <span className="italic text-slate-300">"received 10,000 salary"</span>.
                   </p>
                 </div>
                 {smartVoiceTranscript && !isParsingVoice && (
                   <div className="mt-4 p-4 bg-white/5 border border-white/10 rounded-xl max-w-full">
                     <p className="text-white text-sm italic">"{smartVoiceTranscript}"</p>
                   </div>
                 )}
                 {smartVoiceError && (
          ...
          <form onSubmit={confirmSubmit} className="px-6 pb-6 space-y-5">
            ...
            <div className="grid grid-cols-4 gap-2">
               <button type="button" onClick={() => setType('expense')} className={`py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${type === 'expense' ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/25' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>Out</button>
               <button type="button" onClick={() => setType('income')} className={`py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${type === 'income' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/25' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>In</button>
               <button type="button" onClick={() => setType('udhaar_give')} className={`py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${type === 'udhaar_give' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/25' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>Gave</button>
               <button type="button" onClick={() => setType('udhaar_take')} className={`py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${type === 'udhaar_take' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/25' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>Got</button>
            </div>
                 ...
                 <div className="flex gap-2">
                   ...
                   >
                     ...
                     {currentTypeCategories.map(c => (
                       <option key={c.id} value={c.id}>{t(lang, c.name)}</option>
                     ))}
                   </select>
                   <button 
                     type="button" 
