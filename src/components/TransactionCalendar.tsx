...
export default function TransactionCalendar({ lang, currency, activeContext }: TransactionCalendarProps) {
  ...
  return (
    <div className="flex flex-col h-full space-y-8 animate-in fade-in duration-700 pb-12">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[#1E293B]/40 backdrop-blur-3xl border border-white/10 p-8 rounded-[3rem] relative overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div className="flex items-center bg-[#0F172A]/80 backdrop-blur-md border border-white/10 rounded-[1.5rem] p-1.5 shadow-2xl">
            <button 
              onClick={prevMonth}
              className="p-3 rounded-xl hover:bg-white/5 text-slate-400 hover:text-white transition-all active:scale-90"
            >
              <ChevronLeft size={24} />
            </button>
            <div className="px-8 text-lg font-black text-white min-w-[180px] text-center uppercase tracking-widest tabular-nums">
          ...
          <button 
            ...
            className="px-6 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-[1.5rem] text-sm font-black uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all active:scale-95"
          >
            Today
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <motion.div 
          ...
          className="lg:col-span-8 bg-[#1E293B]/40 backdrop-blur-3xl border border-white/10 rounded-[3rem] p-6 md:p-10 shadow-2xl relative overflow-hidden"
        >
          <div className="grid grid-cols-7 mb-6">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
              <div key={day} className="text-center text-[11px] font-black uppercase tracking-[0.3em] text-slate-500 py-2">
                {day}
              </div>
            ))}
          </div>
            ...
            {calendarDays.map((day, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className={`
                    relative aspect-square flex flex-col p-3 transition-all duration-500 rounded-3xl overflow-hidden
                    ...
                    ${isTodayDay ? 'after:content-[""] after:absolute after:top-3 after:right-3 after:w-2 after:h-2 after:bg-blue-400 after:rounded-full after:shadow-[0_0_15px_#60A5FA] border-blue-500/30' : ''}
                  `}
                >
                  <span className={`
                    text-sm font-black w-8 h-8 flex items-center justify-center rounded-xl transition-all duration-500
                  ...
                  <div className="mt-auto flex flex-col gap-1.5 w-full">
                    {summary.income > 0 && (
                      <div className="h-1.5 md:h-5 w-full bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center px-1.5 overflow-hidden">
                        <div className="hidden md:block text-[9px] font-black text-emerald-400 truncate tracking-tighter">
                          +{formatCurrency(summary.income).split('.')[0]}
                        </div>
                        <div className="md:hidden w-full h-full bg-emerald-400/50 rounded-full" />
                      </div>
                    )}
                    {summary.expense > 0 && (
                      <div className="h-1.5 md:h-5 w-full bg-rose-500/10 border border-rose-500/20 rounded-lg flex items-center px-1.5 overflow-hidden">
                        <div className="hidden md:block text-[9px] font-black text-rose-400 truncate tracking-tighter">
                          -{formatCurrency(summary.expense).split('.')[0]}
                        </div>
                        <div className="md:hidden w-full h-full bg-rose-400/50 rounded-full" />
                      </div>
                    )}
                  </div>
                </motion.button>
