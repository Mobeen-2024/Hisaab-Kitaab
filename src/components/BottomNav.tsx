...
export default function BottomNav() {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#0F172A]/90 backdrop-blur-2xl border-t border-white/10 flex items-center justify-between px-2 pb-safe z-40">
      <NavLink
        to="/"
        className={({ isActive }) => `flex-1 flex flex-col items-center justify-center gap-1 ${isActive ? 'text-blue-400' : 'text-slate-500 hover:text-white'}`}
      >
      ...
      <NavLink
        to="/reports"
        className={({ isActive }) => `flex-1 flex flex-col items-center justify-center gap-1 ${isActive ? 'text-indigo-400' : 'text-slate-500 hover:text-white'}`}
      >
        <FileText size={20} />
      ...
      <NavLink
        to="/customers"
        className={({ isActive }) => `flex-1 flex flex-col items-center justify-center gap-1 ${isActive ? 'text-emerald-400' : 'text-slate-500 hover:text-white'}`}
      >
        <Users size={20} />
